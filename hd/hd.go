...
import (
	...
	"github.com/duminghui/go-wallet/bip39"
	"github.com/duminghui/go-wallet/d/chaincfg"
	"github.com/duminghui/go-wallet/util"
	"github.com/duminghui/go-wallet/util/hdkeychain"
)
...
func (p *PathWrapper) PrivKeyImportable() (string, error) {
	...
	wif, err := util.NewWIF(ecPrivKey, p.net, true)
	if err != nil {
		return "", err
	}
...
func (p *PathWrapper) AddressNativeSegwit() (string, error) {
	...
	p2wkhAddr, err := util.NewAddressWitnessPubKeyHash(pubKeyHash, p.net)
	if err != nil {
		return "", err
	}
...
func (p *PathWrapper) AddressNestedSegwit() (string, error) {
	...
	witnessAddr, err := util.NewAddressWitnessPubKeyHash(util.Hash160(pubKey.SerializeCompressed()), p.net)
